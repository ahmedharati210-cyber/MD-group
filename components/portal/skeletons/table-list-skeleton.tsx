      ...
      <div className="md:hidden space-y-3">
        {Array.from({ length: Math.min(rows, 6) }).map((_, i) => (
          ...
          >
            ...
            <div className="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-xs" />
          </div>
        ))}
      </div>
        ...
        <div className="divide-y divide-gray-100 dark:divide-gray-800">
          <div className="flex gap-4 px-5 py-3 bg-gray-50 dark:bg-gray-800/60">
            {Array.from({ length: columns }).map((_, i) => (
              <div
                key={i}
                className="h-4 bg-gray-200 dark:bg-gray-700 rounded-xs"
          ...
          {Array.from({ length: rows }).map((_, i) => (
            <div key={i} className="flex gap-4 px-5 py-3.5">
              {Array.from({ length: columns }).map((_, j) => (
                <div
                  key={j}
                  className="h-4 bg-gray-100 dark:bg-gray-800 rounded-xs"

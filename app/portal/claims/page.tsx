...
import { Plus, Receipt, CalendarDays, FileText, Trash2 } from "lucide-react";
...
export default async function ClaimsPage() {
  ...
  return (
    <div>
      ...
      {(claims ?? []).length === 0 ? (
        <EmptyState icon={Receipt} title="لا توجد مطالبات" description="لم يتم إضافة مطالبات بعد." />
      ) : (
            ...
            <Link key={c.id} href={`/portal/claims/${c.id}`} className="flex items-start gap-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-4 shadow-sm hover:border-primary-300 dark:hover:border-primary-700 hover:shadow-md transition-all">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-3 mt-2">
                  ...
                  {c.file_url ? (
                    <span className="inline-flex items-center gap-1.5 text-sm text-primary-600 dark:text-primary-400 font-medium">
                      <FileText className="w-4 h-4" /> يحتوي على PDF
                    </span>
                  ) : null}
                </div>
              </div>
